...
func NewRouter(userRepo storage.UserRepository, orderRepo storage.OrderRepository, balanceRepo storage.BalanceRepository, jwtSecret string) http.Handler {
	r := chi.NewRouter()

	r.Use(middleware.Logger)
	r.Use(middleware.GzipHandle)

	h := handlers.NewHandler(userRepo, orderRepo, balanceRepo, jwtSecret)
